...
export async function GET(request: NextRequest, { params }: { params: { subdomain: string; id: string } }) {
  try {
    const { subdomain, id } = params

    // Fetch student portal information
    const student = await db.student.findUnique({
      where: { id },
      select: {
        id: true,
    ...
    console.error('Error fetching student portal info:', error)
    return NextResponse.json({
      success: false,
      error: 'Failed to fetch student portal information'
    }, { status: 500 })
  }
}